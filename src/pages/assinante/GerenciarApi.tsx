  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="bg-[#0b1325] border border-slate-800/80 rounded-3xl p-8 shadow-xl flex flex-wrap items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight flex items-center">
            <KeyRound className="w-6 h-6 mr-2.5 text-emerald-400" />
        ...
        <div className="flex items-center space-x-3">
          <Link
            to="/docs"
            className="inline-flex items-center px-4 py-2.5 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition"
          >
          ...
          <button
            ...
            className="inline-flex items-center px-5 py-2.5 rounded-xl text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white transition shadow-lg shadow-emerald-600/20"
          >
      ...
      <div className="bg-[#0b1325] border border-slate-800/80 rounded-2xl p-6 shadow-xl">
        {loading ? (
          <div className="py-12 text-center text-xs text-slate-500">Carregando chaves de API...</div>
        ) : keys.length === 0 ? (
              ...
              <tbody className="divide-y divide-slate-800/60">
                {keys.map((k) => (
                  <tr key={k.id} className="hover:bg-slate-800/30 transition">
                    ...
                    <td className="py-3 px-4 text-slate-400">{k.rateLimitMin} req/min</td>
                    <td className="py-3 px-4 font-bold text-white">{k.totalCalls}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-[#0b1325] border border-slate-800 rounded-3xl p-8 max-w-md w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-base font-bold text-white">Criar Nova Chave de API</h3>
            ...
            <form onSubmit={handleCreateKey} className="space-y-4 text-xs">
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">IPs Permitidos (Opcional - separador vírgula)</label>
                <input
                  type="text"
                  value={allowedIps}
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Limite de Taxa (Req / Minuto)</label>
                <input
                  type="number"
                  value={rateLimit}
              ...
              <button
                type="submit"
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-3 rounded-xl text-xs transition shadow-lg shadow-emerald-600/20"
              >
                Gerar Chave
              </button>
            </form>
