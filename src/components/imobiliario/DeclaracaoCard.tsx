    ...
    <div className="bg-white border border-slate-200/90 hover:border-slate-300 rounded-2xl p-6 shadow-xs transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 py-4 my-1">
        <div className="bg-slate-50/80 p-3.5 rounded-xl border border-slate-200/70">
          ...
          <p className="text-sm font-bold text-slate-900 font-mono">{declaracao.matricula || 'Não informada'}</p>
        </div>
        ...
        <div className="bg-slate-50/80 p-3.5 rounded-xl border border-slate-200/70 md:col-span-2">
          <span className="text-[11px] font-medium text-slate-500 flex items-center mb-1">
            <Landmark className="w-3 h-3 mr-1 text-blue-600" />
          ...
          <div className="flex items-center space-x-2 text-[11px] text-slate-500 mt-0.5">
            ...
            {declaracao.cnpjCartorio && (
              <>
                <span>•</span>
                <span className="font-mono">CNPJ: {declaracao.cnpjCartorio}</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-slate-100">
        ...
        <div className="space-y-2">
          <div className="flex items-center text-xs font-semibold text-amber-800">
            <Users className="w-3.5 h-3.5 mr-1 text-amber-600" />
            Alienante(s) / Transmitente(s)
          </div>
          {declaracao.alienantes && declaracao.alienantes.length > 0 ? (
            <div className="space-y-1.5">
              {declaracao.alienantes.map((al, idx) => (
                <div key={idx} className="bg-amber-50/40 px-3 py-2 rounded-lg border border-amber-200/60 text-xs">
                  <p className="font-semibold text-slate-900">{al.nome}</p>
                  <p className="text-slate-500 font-mono text-[11px] mt-0.5">Doc: {al.cpfCnpj}</p>
                </div>
              ))}
            </div>
          ) : (
          ...
          {declaracao.adquirentes && declaracao.adquirentes.length > 0 ? (
            <div className="space-y-1.5">
              {declaracao.adquirentes.map((ad, idx) => (
                <div key={idx} className="bg-emerald-50/40 px-3 py-2 rounded-lg border border-emerald-200/60 text-xs">
                  <p className="font-semibold text-slate-900">{ad.nome}</p>
                  <p className="text-slate-500 font-mono text-[11px] mt-0.5">Doc: {ad.cpfCnpj}</p>
                </div>
              ))}
            </div>
          ) : (
